...
class Ina231 {
  ...
  async measure() {
    ...
    const mvBus = await this.adapter.transmit(`AT+TR=${this.address}0202`).then(v => v.readUInt16BE() * 1.25);
    const mwPower = await this.adapter.transmit(`AT+TR=${this.address}0302`).then(v => v.readUInt16BE() * 25 * this.currentLsb * 1000);
    const maShunt = await this.adapter.transmit(`AT+TR=${this.address}0402`).then(v => v.readUInt16BE() * this.currentLsb * 1000);
